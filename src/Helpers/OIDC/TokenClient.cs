...
namespace CommunAxiom.DotnetSdk.Helpers.OIDC
{
    public class TokenClient
    {
        ...
        public async Task<(bool, TokenData?)> AuthenticateClient(string scope, string? clientId = null, string? secret = null)
        {
            await this.Configure();
            ...
            {
                Address = TokenMetadata.TokenEndpoint,
                ClientId = clientId ?? this._settings.ClientId,
                ClientSecret = secret ?? this._settings.Secret,
                Scope = scope
            });
            if (res.HttpResponse.IsSuccessStatusCode)
            {
